...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎨 Add element request from ${session.user.email}:`, {
      filename: image.name,
      size: image.size,
      type: image.type,
    ...
    // Process add element
    const result = await imageEditor.addElementToImage(image, prompt)
    ...
    console.error('❌ Add element API error:', error)
...
export async function GET() {
  return NextResponse.json({
    endpoint: 'Add Element API',
    description: 'Add elements to images using AI',
    methods: ['POST'],
    parameters: {
      image: 'File - Base image file (max 10MB)',
      prompt: 'String - Description of element to add'
    },
